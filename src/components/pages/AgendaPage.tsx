...
export function AgendaPage({ events, isLoading, onRefresh }: AgendaPageProps) {
  ...
  const handleJoinMeeting = (link: string, title: string) => {
    ...
    showNotification("info", `${t("agenda.joining")} "${title}"...`);
  };
  ...
  const handleEventSubmit = async (eventData: {
    ...
    if (result.success) {
      showNotification("success", `${t("agenda.eventCreated")}: "${eventData.title}"`);
      onRefresh?.();
    } else {
      throw new Error(result.error || t("agenda.failedCreate"));
    }
  };
  ...
  const handleDeleteEvent = async (eventId: string, eventTitle: string) => {
    ...
    try {
      ...
      if (result.success) {
        showNotification("success", `${t("agenda.eventDeleted")}: "${eventTitle}"`);
        setExpandedEvent(null);
        onRefresh?.();
      } else {
        showNotification("error", result.error || t("agenda.failedDelete"));
      }
    } catch {
      showNotification("error", t("agenda.failedDelete"));
    } finally {
      setIsDeleting(null);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {(currentEvent || nextEvent) && (
        <div
          className={`rounded-2xl border-2 overflow-hidden ${currentEvent
          ...
          <div className="px-5 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  className={`w-2 h-2 rounded-full ${currentEvent ? "bg-green-500 animate-pulse" : "bg-primary"
                    }`}
                />
                <span className="text-sm font-medium text-muted-foreground">
                  {currentEvent ? t("agenda.happeningNow") : t("agenda.upNext")}
                </span>
              </div>
          ...
          <h2 className="text-lg font-semibold text-foreground flex items-center gap-3">
            ...
            {sortedEvents.length > 0 && (
              <span className="text-xs font-semibold text-muted-foreground bg-muted/20 border border-border/30 px-2.5 py-1 rounded-full">
                {sortedEvents.length} {t("agenda.events")}
              </span>
            )}
          </h2>
          <button
          ...
          <div className="divide-y divide-border">
            {sortedEvents.map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className={`transition-colors ${isPast ? "opacity-50" : ""} ${isNow
                      ? "bg-green-500/5"
                      : isSoon
                        ? "bg-primary/5"
                        : "hover:bg-accent"
                  ...
                  >
                    <div className="flex gap-4">
                      ...
                      <div className="flex-shrink-0 w-20">
                        <div
                          className={`text-sm font-semibold ${isNow
                              ? "text-green-500"
                              : isSoon
                                ? "text-primary"
                                : "text-primary"
